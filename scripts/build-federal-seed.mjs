...
const pfs = await pull(
  'medicare_locality_cpt_rate?state=eq.CA&nonfac_rate=not.is.null&fac_rate=not.is.null&description=not.is.null&select=cpt,description,nonfac_rate,fac_rate&order=cpt',
  'PFS (CA)')
...
const withOpps = catalog.filter((c) => c.opps != null).length
const withAsc = catalog.filter((c) => c.asc != null).length
const withSi = catalog.filter((c) => c.si).length
console.log(`\nwrote ${join(OUT, 'federal-catalog.json')}`)
console.log(`  CPT catalog        : ${catalog.length} codes (ALL REAL)`)
